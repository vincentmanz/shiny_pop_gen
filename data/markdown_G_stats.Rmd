...
```{r G-statistic, tidy = TRUE}
a <- result_f_stats_selec %>% select('Fis (W&C)')

require(tidyverse)
b <- as_tibble(mat) %>% filter(row_number()==n()) %>%  rownames_to_column %>% gather(variable, value, -rowname) %>%  spread(rowname, value) %>%  slice(1:(n() - 1)) %>% column_to_rownames('variable')
colnames(b) <- ('Missing %')
...
fis_df_sg %>% mutate(`Two-sided p-values` = ifelse(smaller > 0.5, 2 * (1 - smaller), 2 * smaller))
